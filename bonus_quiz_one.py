...
# Розміри кнопок відповідей
BUTTON_WIDTH = 555
BUTTON_HEIGHT = 60
FEEDBACK_DELAY = 800  # мс пауза між питаннями
...
questions = [
    ...
    ("Як називається вибух зірки?", ["Супернова", "Комета", "Пульсар"], 0),
    ("Залишки метеорита на Землі — це...", ["Астероїди", "Метеорити", "Комети"], 1),
    ("Темна частина чорної діри — це...", ["Горизонт подій", "Центр", "Квантова зона"], 0),
    ("Найближча галактика до нашої?", ["Андромеда", "Мала Хмара", "Оріон"], 0),
    ("Який корабель доставив людей\nна Місяць?", ["Аполлон-11", "Союз-1", "Вояджер-1"], 0),
]
...
def draw_question(q_index, selected):
    ...
    # --- Прямокутник для питання ---
    question_rect = pygame.Rect(300, 170, 600, 120)

    # Розбивка питання на рядки
    words = q.replace('\n', ' ').split(' ')
    lines = []
    current_line = ""
    ...
    # Малювання питання по центру прямокутника
...
def bonus_quiz(music_volume=0.5, sfx_volume=0.5):
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if selected is None and event.type == pygame.MOUSEBUTTONDOWN:
                mx, my = pygame.mouse.get_pos()
        ...
        if selected is not None:
            ...
            if pygame.time.get_ticks() - feedback_timer > FEEDBACK_DELAY:
                ...
                if current_question >= len(questions):
                    res = end_menu(score >= 10, background_image)
                    if res == "restart":
